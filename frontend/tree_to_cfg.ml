...
let create_node (pos : position) =
  incr node_counter ;
  let node =
    {node_id= !node_counter; node_pos= pos; node_in= []; node_out= []}
  in
  nodes := node :: !nodes ;
  node
...
let add_arc (src : node) (dst : node) (inst : inst) =
  incr arc_counter ;
  let arc =
    {arc_id= !arc_counter; arc_src= src; arc_dst= dst; arc_inst= inst}
  in
  src.node_out <- arc :: src.node_out ;
  dst.node_in <- arc :: dst.node_in ;
  (* remember call sites for call instructions *)
  (match inst with CFG_call f -> f.func_calls <- arc :: f.func_calls | _ -> ()) ;
  arcs := arc :: !arcs
...
let create_var (name : string) (pos : extent) (typ : typ) =
  incr var_counter ;
  {var_id= !var_counter; var_name= name; var_pos= pos; var_type= typ}

let fun_counter = ref 0

(* create a function structure, assigning it a fresh identifier *)
let create_fun (name : string) (entry : node) (exit : node) (pos : extent)
    (args : var list) (ret : var option) =
  incr fun_counter ;
  { func_id= !fun_counter
  ; func_name= name
  ; func_pos= pos
...
let rec int_expr (env : env) (expr : AbstractSyntax.int_expr) :
  ...
  | AST_int_const (i, x) ->
      let v =
        ...
        with _ ->
          failwith
            (Printf.sprintf "invalid integer constant %s at %s" i
               (string_of_extent x) )
      in
      (env, [], CFG_int_const v)
  | AST_int_rand ((i1, x1), (i2, x2)) ->
      let v1 =
        ...
        with _ ->
          failwith
            (Printf.sprintf "invalid integer constant %s at %s" i1
               (string_of_extent x1) )
      and v2 =
        try Z.of_string i2
        with _ ->
          failwith
            (Printf.sprintf "invalid integer constant %s at %s" i2
               (string_of_extent x2) )
      in
  ...
  | AST_expr_call ((id, x), exprs) -> (
      ...
      | None ->
          failwith
            (Printf.sprintf "function %s has no return value at %s" id
               (string_of_extent x) )
      | Some var ->
...
and bool_expr (env : env) (expr : AbstractSyntax.bool_expr) :
...
and call (env : env) ((id, x) : id ext)
    (exprs : AbstractSyntax.int_expr ext list) : env * inst ext list * func =
  let f =
    try StringMap.find id env.env_funcs
    with Not_found ->
      failwith
        (Printf.sprintf "unknown function %s at %s" id (string_of_extent x))
  in
  (* match formal and actual arguments *)
  let rec doargs env inst args =
    match args with
    | [], [] ->
        (env, inst)
    ...
    | _ ->
        failwith
          (Printf.sprintf "wrong number of arguments for function %s at %s" id
             (string_of_extent x) )
  in
...
let rec stat (env : env) (entry : node) (exit : node) (s : stat) : env =
  ...
  | AST_increment ((id, x), v) ->
      ...
      add_arc entry exit
        (CFG_assign
           ( var
           , CFG_int_binary
               (AST_PLUS, CFG_int_var var, CFG_int_const (Z.of_int v)) ) ) ;
      env
  ...
  | AST_break ((), x) ->
      ...
      | Some node ->
          add_arc entry node (CFG_skip "skip: break")
      | None ->
          failwith
            (Printf.sprintf "break outside loop at %s" (string_of_extent x)) ) ;
      env
  | AST_return None ->
      ...
      | Some exit ->
          add_arc entry exit (CFG_skip "skip: return")
      | None ->
          failwith "no exit node for function" ) ;
      env
  | AST_return (Some (expr, x)) ->
      ...
      let var =
        ...
        | None ->
            failwith
              (Printf.sprintf "function cannot return a value at %s"
                 (string_of_extent x) )
      in
      ( match env1.env_exit with
      | Some exit ->
          add_arc entry1 exit (CFG_assign (var, e))
      | None ->
          failwith "no exit node for function" ) ;
      env1
  ...
  | AST_label (id, x) ->
      ...
      if StringMap.mem id env.env_labels then
        failwith
          (Printf.sprintf "duplicate label %s at %s" id (string_of_extent x)) ;
...
let func (env : env) (f : fun_decl) : env =
  ...
  let ret =
    match f.fun_typ with
    | None, _ ->
        None
    | Some t, _ ->
        Some (create_var ("__return_" ^ fst f.fun_name) f.fun_ext t)
  in
  ...
  List.iter
    (fun (src, (id, x)) ->
      let dst =
        ...
        with Not_found ->
          failwith
            (Printf.sprintf "unknown label %s at %s" id (string_of_extent x))
      in
      add_arc src dst (CFG_skip ("skip: goto " ^ id)) )
    env4.env_gotos ;
  (* returned environment *)
  {env with env_funcs= env4.env_funcs; env_allvars= env4.env_allvars}
...
let prog ((t, x) : prog) : cfg =
  ...
  let vars =
    List.rev (VarSet.fold (fun a acc -> a :: acc) env.env_allvars [])
  in
  let funcs =
    List.rev (StringMap.fold (fun _ f acc -> f :: acc) env.env_funcs [])
  in
  { cfg_vars= vars
  ; cfg_funcs= funcs
